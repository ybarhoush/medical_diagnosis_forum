...
from .utils import MESSAGES_TABLE, INITIAL_MESSAGES_COUNT
...
MESSAGE_2 = {
    ...
    'body': ("Hi, I need help with this issue real quick. I get very dizzy when I run for "
             "few minutes. This is being happening for like 2 weeks now. I really need help "
             "with this ! "),
...
class DatabaseMessagesTestCase(unittest.TestCase):
    ...
    def test_create_message_object(self):
        ...
        # Test the method
        message = self.connection._create_message_object(
            execute_query(self, query, 'one'))
    ...
    def test_get_message(self):
        ...
        self.assertDictContainsSubset(
            self.connection.get_message(MESSAGE_1_ID), MESSAGE_1)
        self.assertDictContainsSubset(
            self.connection.get_message(MESSAGE_2_ID), MESSAGE_2)

    def test_get_message_bad_id(self):
        """
        Test get_message with id 1 (malformed) and not 'msg-1'
        """
        print('(' + self.test_get_message_bad_id.__name__+')',
              self.test_get_message_bad_id.__doc__)
    ...
    def test_get_message_non_exist_id(self):
        ...
        """
        print('(' + self.test_get_message_non_exist_id.__name__+')',
              self.test_get_message_non_exist_id.__doc__)
    ...
    def test_get_messages_length(self):
        ...
        messages = self.connection.get_messages(
            username=MESSAGE_DUMMY_USERNAME, number_of_messages=2)
    ...
    def test_delete_message_bad_id(self):
        ...
        """
        print('(' + self.test_delete_message_bad_id.__name__+')',
              self.test_delete_message_bad_id.__doc__)
        with self.assertRaises(ValueError):
            self.connection.delete_message(BAD_MESSAGE_ID)

    def test_delete_nonexist_messageid(self):
        """
        Test delete_message with a message that doesn't exist (msg-256)
        """
        print('(' + self.test_delete_nonexist_messageid.__name__+')',
              self.test_delete_nonexist_messageid.__doc__)
    ...
    def test_modify_message(self):
        ...
        resp = self.connection.modify_message(
            MESSAGE_1_ID, "new title", "new body")
        self.assertEqual(resp, MESSAGE_1_ID)
        # check whether modification was successful
        self.assertDictContainsSubset(
            self.connection.get_message(MESSAGE_1_ID), MESSAGE_1_MODIFIED)

    def test_modify_message_bad_id(self):
        """
        Test that trying to modify message with id (1) and not (msg-1) which actually exist
        """
        print('(' + self.test_modify_message_bad_id.__name__+')',
              self.test_modify_message_bad_id.__doc__)
        with self.assertRaises(ValueError):
            self.connection.modify_message(
                BAD_MESSAGE_ID, "new title", "new body")

    def test_modify_nonexist_messageid(self):
        """
        Test that trying to modify message with id (msg-256) which does not exist
        """
        print('(' + self.test_modify_nonexist_messageid.__name__+')',
              self.test_modify_nonexist_messageid.__doc__)
        resp = self.connection.modify_message(
            NON_EXIST_MESSAGE_ID, "new title", "new body")
    ...
    def test_create_message(self):
        ...
        message_id = self.connection.create_message(
            "new title", "new body", MESSAGE_DUMMY_USERNAME)
        ...
        self.assertDictContainsSubset(
            real_message, self.connection.get_message(message_id))
    ...
    def test_append_answer(self):
        ...
        message_id = self.connection.append_answer(
            MESSAGE_1_ID, "new title", "new body", MESSAGE_DUMMY_USERNAME)
        self.assertIsNotNone(message_id)
    ...
    def test_append_answer_malformed_id(self):
        ...
        with self.assertRaises(ValueError):
            self.connection.append_answer(
                BAD_MESSAGE_ID, "new title", "new body", MESSAGE_DUMMY_USERNAME)

    def test_append_answer_non_exist_id(self):
        """
        Test append_answer with  id (msg-256) (no-existing)
        """
        print('(' + self.test_append_answer_non_exist_id.__name__+')',
              self.test_append_answer_non_exist_id.__doc__)
        self.assertIsNone(self.connection.append_answer(
            NON_EXIST_MESSAGE_ID, "new title", "new body", MESSAGE_DUMMY_USERNAME))

    def test_contains_valid_message(self):
        """
        Check if the database contains messages with id msg-1 and msg-8
        """
        print('(' + self.test_contains_valid_message.__name__+')',
              self.test_contains_valid_message.__doc__)
    ...
    def test_not_contains_message(self):
        ...
        """
        print('(' + self.test_not_contains_message.__name__+')',
              self.test_not_contains_message.__doc__)
        # the right response should be false otherwise assert error
        self.assertFalse(
            self.connection.contains_message(NON_EXIST_MESSAGE_ID))
